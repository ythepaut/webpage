...
export default function Projects(): JSX.Element {
    ...
    return (
        <section id="projects" className="bg-white py-10 lg:p-20">
            <ContentTransition>
                <>
                    <h1 className="sm:text-4xl text-2xl font-medium title-font text-gray-900 pb-6 text-center">{t("title")}</h1>
            ...
            <div className="container px-5 mx-auto">
                <ContentTransition>
                    <div className="flex flex-wrap">
                        {projects
                            ? projects.map((project: any, index) => <ProjectCard key={index} project={project} />)
                            : Array.from({ length: 4 }, (_, i) => i + 1).map((i) => <ProjectCardSkeleton key={i} />)}
                    </div>
                </ContentTransition>
            </div>
        </section>
